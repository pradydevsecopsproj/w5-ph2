# EKS Managed Node Group - Public Subnets   --- ## for this demo only using Public subnet, so commented. In prod we ned to use Private subnets for hosting workloads
resource "aws_eks_node_group" "public_nodes" {
  ...
  node_group_name = "${local.name}-public-ng-prady"
  # IAM role that EC2 worker nodes will assume
  node_role_arn = aws_iam_role.eks_nodegroup_role.arn

  # Subnets where the worker nodes will be launched (typically private subnets) ## for this demo only using Public subnet, so commented. In prod we ned to use Private subnets for hosting workloads
  # subnet_ids = data.terraform_remote_state.vpc.outputs.private_subnet_ids

  # Subnets where the worker nodes will be launched (typically public subnets)  ## for this demo only using Public subnet for hosting workloads
  ...
  scaling_config {
    ...
    desired_size = 2 # for demo purpose to save cost crating 2 node. we can create 3 nodes
  ...
  tags = merge(var.tags, {
    ...
    # Name = "${local.name}-private-ng" ## for this demo only using Public subnet for hosting workloads
